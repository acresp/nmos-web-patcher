# /services/logical.py
# by Arnaud Cresp - 2025

import json
from pathlib import Path

LOGICAL_FILE = Path("data_logical.json")

def load_logical_ids():
    if LOGICAL_FILE.exists():
        with open(LOGICAL_FILE, "r") as f:
            data = json.load(f)

        if "sources" in data and "senders" not in data:
            data["senders"] = data.pop("sources")

        data.setdefault("senders", {})
        data.setdefault("receivers", {})

        return data
    return {"senders": {}, "receivers": {}}

def save_logical_ids(data):
    clean = {
        "senders": data.get("senders", {}),
        "receivers": data.get("receivers", {})
    }

    with open(LOGICAL_FILE, "w") as f:
        json.dump(clean, f, indent=2)

def get_logical_pair(src_id, dest_id):
    logical = load_logical_ids()

    senders = logical.get("senders", {})
    receivers = logical.get("receivers", {})

    src = next((v for v in senders.values() if int(v.get("id", -1)) == int(src_id)), None)
    dest = next((v for v in receivers.values() if int(v.get("id", -1)) == int(dest_id)), None)

    return src, dest